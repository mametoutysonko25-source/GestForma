...
$requete = $db->prepare('SELECT s.idSeance, s.idModule, s.dateSeance, s.heureDebut, s.heureFin, s.statut, m.libelle AS module FROM seance s INNER JOIN module m ON m.idModule = s.idModule WHERE s.idFormateur = :id ORDER BY s.dateSeance DESC, s.heureDebut DESC');
$requete->execute(['id' => $formateurId]);
...
{
    if ($seance['dateSeance'] !== date('Y-m-d')) return false;
    $debut = strtotime($seance['dateSeance'] . ' ' . $seance['heureDebut']);
    return time() >= $debut && time() <= $debut + 900;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && $seance) {
    if (!appelEstOuvert($seance)) {
        $messageErreur = 'La feuille d’appel est disponible uniquement pendant les 15 premières minutes du cours.';
    } else {
...
?>
<div class="director-heading"><div><h2>Feuille d’appel</h2><p>Cochez la présence de chaque étudiant pendant les 15 premières minutes du cours.</p></div></div>
...
<?php if ($seance): ?><div class="attendance-banner <?= $appeluOuvert?'attendance-open':'attendance-closed' ?>"><strong><?= $appeluOuvert?'Appel ouvert':'Appel fermé' ?></strong><span><?= $appeluOuvert?'Vous pouvez encore enregistrer les présences.':'La fenêtre de 15 minutes est dépassée ou la séance n’a pas lieu aujourd’hui.' ?></span></div>
