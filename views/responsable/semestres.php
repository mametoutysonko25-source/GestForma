    ...
    if ($action === 'supprimer' && isset($_POST['idSemestre'])) {
        ...
        if ($idNiveau <= 0 || $libelle === '') {
            ...
            $statement = database()->prepare(
                "UPDATE semestre SET idNiveau = :idNiveau, libelle = :libelle, ordre = :ordre, description = :description
                 WHERE idSemestre = :id"
            );
            $statement->execute([
                'idNiveau' => $idNiveau, 'libelle' => $libelle, 'ordre' => $ordre,
                'description' => $description ?: null, 'id' => $idSemestre,
            ]);
            ...
            $statement->execute([
                'idNiveau' => $idNiveau, 'libelle' => $libelle, 'ordre' => $ordre, 'description' => $description ?: null,
            ]);
...
$semestres = database()->query(
    "SELECT s.idSemestre, s.libelle, s.ordre, s.description,
            n.libelle AS niveauLibelle, f.libelle AS formationLibelle,
            COUNT(m.idModule) AS nbModules
     FROM semestre s
     ...
     GROUP BY s.idSemestre, s.libelle, s.ordre, s.description, n.libelle, f.libelle
    ...
    <table style="width:100%; border-collapse:collapse;">
        <thead>
            <tr style="background:var(--primary); color:#fff;">
                ...
                <th style="padding:10px; text-align:left;">Modules</th>
        ...
        <tbody>
            <?php foreach ($semestres as $semestre): ?>
                <tr style="border-bottom:1px solid #eee;">
                    ...
                    <td style="padding:10px;"><?= (int) $semestre['nbModules'] ?></td>
            ...
            <?php endforeach; ?>
            <?php if (!$semestres): ?>
                <tr><td colspan="6" style="padding:16px; text-align:center; color:var(--muted);">Aucun semestre enregistré.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
...
<div class="card" style="max-width:480px;">
    ...
    <form method="POST">
        ...
        <div class="form-field">
            ...
            <?php if (!$niveaux): ?>
                <p style="font-size:12px; color:#c62828; margin:4px 0 0;">Aucun niveau n'existe encore : créez-en un d'abord dans <a href="niveaux.php" style="color:#c62828;">Niveaux</a>.</p>
            <?php endif; ?>
        </div>
        <div class="form-field">
        ...
        <div class="form-field">
            ...
            <input type="text" id="description" name="description" value="<?= htmlspecialchars((string) ($formulaire['description'] ?? '')) ?>">
        </div>
